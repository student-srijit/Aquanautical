...
export function VoiceAgent({ isCallActive, isMuted, isRecording, onEndCall }: VoiceAgentProps) {
    ...
    if (typeof window !== "undefined" && window.speechSynthesis) {
      ...
      const loadVoices = () => {
        ...
        if (voices.length > 0) {
          ...
          console.log("[Voice Agent] Voices loaded:", voices.length, "voices available")
        }
      }
  ...
  const forceInterruptAI = useCallback(() => {
    console.log("[Voice Agent] Force interrupting AI speech")
    if (synthesisRef.current && isCurrentlySpeaking.current) {
      synthesisRef.current.cancel()
      setIsSpeaking(false)
      isCurrentlySpeaking.current = false
      console.log("[Voice Agent] AI speech force interrupted")
    }
  }, [])
  ...
  const startRecognition = useCallback(() => {
    ...
    try {
      ...
      console.log("[Voice Agent] Recognition start failed:", error)
  ...
  const startInterruptionMonitoring = useCallback(() => {
    ...
    interruptionCheckInterval.current = setInterval(() => {
      ...
      if (isCallActive && !isRecognitionRunning.current && !isMuted && !isSpeaking && !isProcessing) {
        console.log("[Voice Agent] Speech recognition stopped unexpectedly - restarting")
        startRecognition()
      }
      ...
      if (isCallActive && isCurrentlySpeaking.current && !isRecognitionRunning.current) {
        console.log("[Voice Agent] AI speaking but recognition not running - potential interruption failure")
  ...
  const startSilenceTimeout = useCallback(() => {
    ...
    if (!isSpeaking && !isProcessing && !isCurrentlySpeaking.current) {
      silenceTimeoutRef.current = setTimeout(() => {
        console.log("[Voice Agent] 12-second silence timeout - AI will speak")
  ...
  const handleAIResponse = useCallback(async (text: string) => {
    ...
    console.log("[Voice Agent] Speaking AI response:", text)
    ...
    if (endingPhrases.some(phrase => lowerText.includes(phrase))) {
      console.log("[Voice Agent] AI detected call ending phrase")
    ...
    if (isCurrentlySpeaking.current) {
      console.log("[Voice Agent] AI is already speaking, queuing new response")
      return
    }
    ...
    try {
      ...
      console.log("[Voice Agent] Speech cancellation completed")
    }
    ...
    utterance.onstart = () => {
      console.log("[Voice Agent] AI started speaking")
      setIsSpeaking(true)
      isCurrentlySpeaking.current = true
    }

    utterance.onend = () => {
      console.log("[Voice Agent] AI finished speaking")
      setIsSpeaking(false)
      isCurrentlySpeaking.current = false
    ...
    utterance.onerror = (event) => {
      if (event.error !== "interrupted") {
        console.error("[Voice Agent] Speech synthesis error:", event.error)
      }
      setIsSpeaking(false)
      isCurrentlySpeaking.current = false
    ...
    if (preferredVoice) {
      ...
      console.log("[Voice Agent] Using voice:", preferredVoice.name)
    }

    setTimeout(() => {
      if (!isCallActive) {
        console.log("[Voice Agent] Call ended, cancelling speech")
        setIsSpeaking(false)
        isCurrentlySpeaking.current = false
        return
      }
      ...
      try {
        ...
        console.error("[Voice Agent] Failed to speak:", error)
        setIsSpeaking(false)
      }
    }, 100)
  ...
  const handleUserSpeech = async (transcript: string) => {
    ...
    console.log("[Voice Agent] Processing user message:", transcript)
    ...
    if (endingPhrases.some(phrase => lowerTranscript.includes(phrase))) {
      console.log("[Voice Agent] User requested to end the call")
      const farewellMessages = [
        "Thank you for this wonderful conversation about deep sea research. Until next time, keep exploring the mysteries of the ocean!",
        "It's been a pleasure discussing marine biology with you. I hope you learned something fascinating about our deep sea world!",
        "What an engaging conversation about the ocean depths! I look forward to our next chat about marine science.",
    ...
    try {
      ...
      console.log("[Voice Agent] AI response received:", aiResponse)
      handleAIResponse(aiResponse)
    } catch (error) {
      console.error("[Voice Agent] Error getting AI response:", error)
  ...
  const initializeSpeechRecognition = useCallback(() => {
    ...
    if (!SpeechRecognition) {
      console.error("[Voice Agent] Speech recognition not supported")
      return
    }
    ...
    recognition.onstart = () => {
      console.log("[Voice Agent] Speech recognition started")
      setIsListening(true)
    ...
    recognition.onresult = (event) => {
      ...
      if (finalTranscript || interimTranscript) {
        console.log("[Voice Agent] User started speaking - interrupting AI")
        ...
        if (synthesisRef.current && isCurrentlySpeaking.current) {
          ...
          setIsSpeaking(false)
          isCurrentlySpeaking.current = false
          console.log("[Voice Agent] AI speech interrupted by user (fallback)")
        }
      ...
      if (finalTranscript.trim()) {
        console.log("[Voice Agent] User said:", finalTranscript)
    ...
    recognition.onerror = (event) => {
      console.log("[Voice Agent] Speech recognition error:", event.error)
  ...
  useEffect(() => {
    if (isCallActive) {
      ...
      callEndTimeoutRef.current = setTimeout(() => {
        console.log("[Voice Agent] 5 minutes of inactivity - AI ending call")
        const farewellMessages = [
          "Thank you for this wonderful conversation about deep sea research. Until next time, keep exploring the mysteries of the ocean!",
          "It's been a pleasure discussing marine biology with you. I hope you learned something fascinating about our deep sea world!",
          "What an engaging conversation about the ocean depths! I look forward to our next chat about marine science.",
      ...
      if (synthesisRef.current) {
        synthesisRef.current.cancel()
        console.log("[Voice Agent] Speech synthesis cancelled on call end")
      }
